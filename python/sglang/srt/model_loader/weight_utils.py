...
def initialize_dummy_weights(
    ...
    """
    Fill all floating params from a NumPy RNG. For nn.Linear:
      - generate canonical (in, out) then store its transpose (out, in).
    All other floating tensors are filled in-place with their own shape.
    Values depend only on (numel, dtype, seed).
    """
    processed = set()
    from sglang.srt.layers.linear import LinearBase

    def _np_gen_dtype_for_torch(tdtype: torch.dtype) -> np.dtype:
    ...
    def _np_fill_uniform(shape, low, high, seed, gen_dtype):
        rng = np.random.default_rng(seed)            # reseed per parameter
    ...
    for prefix, mod in model.named_modules():
        if isinstance(mod, LinearBase):
            ...
            w_gen_dtype = _np_gen_dtype_for_torch(w.dtype)
            W_canon = _np_fill_uniform((in_f, out_f), low, high, seed, w_gen_dtype)  # (in,out)
            W_store = torch.from_numpy(W_canon.T).to(device=w.device, dtype=w.dtype) # (out,in)
            with torch.no_grad():
                w.copy_(W_store)
            processed.add(f"{prefix+'.' if prefix else ''}weight")

            if b is not None:
                b_gen_dtype = _np_gen_dtype_for_torch(b.dtype)
                b_np = _np_fill_uniform((out_f,), low, high, seed, b_gen_dtype)
                with torch.no_grad():
                    b.copy_(torch.from_numpy(b_np).to(device=b.device, dtype=b.dtype))
                processed.add(f"{prefix+'.' if prefix else ''}bias")
    ...
    for name, tensor in model.state_dict().items():
        ...
        arr = _np_fill_uniform(tuple(tensor.shape), low, high, seed, gen_dtype)
        t = torch.from_numpy(arr).to(device=tensor.device, dtype=tensor.dtype)
        with torch.no_grad():
            tensor.copy_(t)
